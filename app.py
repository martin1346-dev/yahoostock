...
# --- 頁面設定 ---
st.set_page_config(page_title="進階投資組合回測系統", layout="wide")
st.title("📈 投資組合複合報酬率 & 風險分析")

# --- 側邊欄參數設定 ---
st.sidebar.header("1. 設定投資標的")
tickers_input = st.sidebar.text_input("輸入股票代碼 (逗號隔開)", value="1326.TW, 2317.TW, 2882.TW, 2330.TW")

st.sidebar.header("2. 回測設定")
years = st.sidebar.slider("回測年數", min_value=1, max_value=40, value=20)
monthly_investment = st.sidebar.number_input("每月定期定額金額 (元)", value=1000, step=500)

dd_threshold = st.sidebar.slider("設定回撤深度警告線 (%)", min_value=10, max_value=70, value=30, step=1)
rf_rate = st.sidebar.number_input("無風險利率設定 (%)", value=2.0, step=0.5) / 100
...
if ticker_list:
    ...
    for ticker in ticker_list:
        w = st.sidebar.number_input(f"{ticker} 權重", 0, 100, 100 // len(ticker_list))
...
if not ticker_list:
    ...
    if error_msg:
        ...
        st.plotly_chart(fig1, width="stretch", theme=None if selected_template else "streamlit")
        ...
        st.plotly_chart(fig2, width="stretch", theme=None if selected_template else "streamlit")
        ...
        if len(ticker_list) > 1:
            ...
            with col_heat:
                ...
                st.plotly_chart(fig_heat, width="stretch", theme=None if selected_template else "streamlit")
            ...
            with st.expander("🧮 AI 演算法建議：尋找最佳權重 (Markowitz 效率前緣)"):
                ...
                if st.button("🚀 開始模擬最佳權重"):
                    with st.spinner("正在模擬限制權重下的 2,000 種投資組合，請稍候..."):
                        ...
                        st.plotly_chart(fig3, width="stretch", theme=None if selected_template else "streamlit")
        ...
        with st.expander("點擊展開：基於歷史數據推演未來可能性 (幾何布朗運動)"):
            ...
            with c_sim1:
                sim_years = st.slider("預測未來年數", min_value=1, max_value=30, value=10)
            with c_sim2:
                initial_capital = st.number_input("模擬初始投入資金 (元)", value=100000, step=10000)
            ...
            if st.button("🎲 執行未來走勢模擬 (運算 1,000 次)"):
                with st.spinner(f"正在以矩陣運算推演未來 {sim_years} 年的 1,000 種宇宙走勢..."):
                    ...
                    st.plotly_chart(fig_mc, width="stretch", theme=None if selected_template else "streamlit")
            ...
            if len(ticker_list) > 1:
                ...
                if st.button("🔄 執行再平衡回測"):
                    with st.spinner("正在逐年計算年底的權重重置..."):
                        ...
                        st.plotly_chart(fig_reb, width="stretch", theme=None if selected_template else "streamlit")
            else:
        ...
        with st.expander("點擊展開：檢驗「無論何時進場，只要抱滿 N 年」的勝率與平均績效"):
            roll_years = st.slider("設定持有年數 (滾動視窗)", min_value=1, max_value=10, value=3)
